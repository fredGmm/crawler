...
def down_article(cache):
    ...
    while True:
        ...
        if url:
            ...
            article_html = WebPageDown.down_web_page_html('https://bbs.hupu.com/20188181.html', {'User-agent': 'fred_sq'}, proxy=None, retry=1)
...
if __name__ == "__main__":
    ...
    # url = 'https://bbs.hupu.com/lol'
    # pg = Process(target=grab_url, args=(1,))
    #
    # pg.start()  #抓取页面 url 进程


    dr_processes = []
    for i in range(1): #抓取页面进程
        dr = Process(target=down_article, args=(1,))
        dr.start()
