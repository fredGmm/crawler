...
db = MysqlConn(host='192.168.1.111', port=3306, user='root', passwd='xiaoming', db='web_data')
sql = "select id,article_id,images from hupu_article_info where images != '[]' order by id limit 0,100"
result = db.find_article(sql)
...
for article in list(result):
    ...
    for image in image_list:
        url = image
        if 'jpg' in url or url.find('jpeg') or url.find('png'):
            # url = url.strip('" "')
            # url = "https://t12.baidu.com/it/u=2388568829,148893518&fm=173&s=01725C908A5108C81A07A8C3030070AB&w=640&h=1580&img.JPEG"
            # url = "https://i10.hoopchina.com.cn/hupuapp/bbs/854/71064499034854/thread_71064499034854_20180131020727_s_160656_h_1334px_w_750px1018260953.jpeg?x-oss-process=image/resize"
